    ...
    while (True):
        ...
        if (epoch % config.valid_iter == 0):
            ...
            if (flag):
                ...
                model.save(os.path.join(config.model_save_path, f"02.06_drop0.2_{epoch}_{eav_ppl}_checkpoint.pth"))
                torch.save(optimizer.optimizer.state_dict(), os.path.join(config.model_save_path, f"02.06_drop0.2_{epoch}_{eav_ppl}_optimizer.optim"))
