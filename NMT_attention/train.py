...
def evaluate_ppl(model, dev_data_src, dev_data_tar, batch_size):
    flag = model.training
    model.eval()
    ...
    with torch.no_grad():
        for src, tar in utils.batch_iter(dev_data_src, dev_data_tar, batch_size):
            loss = -model(src, tar)
            loss = loss.sum()
            for sent in tar:
                batch_size += len(sent) - 1
            batch_loss += loss
...
def train():
    ...
    while (True):
        epoch += 1
        for src_sents, tar_sents in utils.batch_iter(train_data_src, train_data_tar, config.batch_size):
            train_iter += 1
            optimizer.zero_grad()
            ...
            tar_words_num_to_predict = 0
            for sents in tar_sents:
                tar_words_num_to_predict += len(sents) - 1
            ...
            if (train_iter % config.valid_iter == 0):
